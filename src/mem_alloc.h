 ...
 * @brief Custom memory allocation interface replicating standard library
 * functions.
 * @version 1.0.0
 *
 * This header provides a custom implementation of memory management functions
 * similar to malloc, free, realloc, and calloc from the standard library.
 */
 ...
 * @warning The returned pointer must be freed with mem_free() when no longer
 * needed.
 */
 ...
 * @warning Passing an invalid pointer (not allocated by mem_alloc/mem_resize)
 * or a pointer that was already freed results in error.
 */
 ...
 * @retval NULL If allocation failed (out of memory), original block remains
 * valid.
